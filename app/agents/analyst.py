"""Test Response Analyst Agent — uses Ollama to produce AI risk assessment.

System prompt is assembled from:
  .github/agents/test-response-analyst.agent.md  (persona + approach)
  .github/skills/api-test-reporting/SKILL.md      (reporting procedure)
  .github/skills/api-batch-triage/SKILL.md        (triage procedure)
...
from app.integrations.ollama_client import _extract_json
...
class TestResponseAnalystAgent(BaseOllamaAgent):
    """Analyses a completed batch of test results and returns a structured risk report.

        Persona and procedure are loaded from the analyse use case, which includes:
            .github/agents/api-test-orchestrator.agent.md
            .github/agents/test-response-analyst.agent.md
            .github/skills/api-test-reporting/SKILL.md
            .github/skills/api-batch-triage/SKILL.md
    """
    ...
    def analyse(self, groups: list[dict]) -> dict:
        """Produce an AI risk assessment from a list of test result groups.

        Each group has: api_request, generated, test_results, summary (optional).
        Returns a structured dict with risk_level, findings, recommendations.
        """
                    ...
                    failed_cases.append(
                        {
                            ...
                            "actual_status": r.get("actual_status"),
                            "assertion_notes": r.get("assertion_notes", ""),
                            "failure_suggestion": r.get("failure_suggestion", ""),
                        }
                    )
                else:
                    passed += 1
        ...
        if total == 0:
            return {
                "risk_level": "clean",
                "summary": "No test results to analyse.",
                "findings": [],
                "recommendations": [],
                "stats": stats,
            }
        ...
        # Fallback: return stats without AI narrative
        ...
        return {
            "risk_level": risk,
            "summary": f"{passed}/{total} tests passed. {failed} failed, {errors} errored.",
            "findings": [],
