...
def run_one(tc: dict) -> dict:
    """Execute a single test case. Never raises - errors are captured in result."""
    name = tc.get("name", "unnamed")
    method = str(tc.get("method", "GET")).upper()
    url = tc.get("url", "")
    log.debug("[Runner] %s %s — %s", method, url, name)
    headers: dict = dict(tc.get("headers") or {})
    payload: Any = tc.get("payload") or {}
    ...
    try:
        ...
        if result["passed"]:
            log.debug("[Runner] PASS %s %s — %s (%dms)", method, url, name, elapsed)
        else:
            log.info(
                "[Runner] FAIL %s %s — %s | expected=%d actual=%d (%dms)",
                method, url, name, expected_status, resp.status_code, elapsed,
            )
...
def run_all(test_cases: list[dict]) -> list[dict]:
    """Run every test case and return all results."""
    log.info("[Runner] Running %d test case(s)", len(test_cases))
    results = [run_one(tc) for tc in test_cases]
    passed = sum(1 for r in results if r.get("passed"))
    errors = sum(1 for r in results if r.get("error"))
    log.info("[Runner] Finished — passed=%d failed=%d errors=%d", passed, len(results) - passed - errors, errors)
    return results


def summarise(results: list[dict]) -> dict:
    """Return pass/fail/error counts for a results list."""
    passed = sum(1 for r in results if r.get("passed"))
    errors = sum(1 for r in results if r.get("error"))
